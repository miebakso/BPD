	...
	open cursorPromoValid
		...
		begin
			...
			open cursorPromoValidCheck
				...
				declare @connectedPromoByTopping table(
					idPromo int
				)
				
				declare @toppingCur1 table(
					idTopping int
				)

				declare @toppingCur2 table(
					idTopping int
				)
				...
				while (@@FETCH_STATUS=0)begin
					...
					delete from @toppingCur1
					delete from @toppingCur2

					insert into @toppingCur1
					select
						idTopping
					from
						@promoValid
					...
					--select idTopping as 'satu' from @toppingCur1
					--select idTopping as 'dua' from @toppingCur2

					select
						@isEmpty = count(pertama.idTopping)
					from
						@toppingCur1 as pertama JOIN @toppingCur2 as kedua
					on
						pertama.idTopping = kedua.idTopping
						
					--select @isEmpty
					--Cek apakah topping pada cursor kedua ini memuat topping pada cursor pertama
					if(@isEmpty = 0)begin
						delete from @connectedPromoByTopping
						/*Cari idPromo mana saja yang akan dihapus karena memuat 
							topping pada promo baru yang akan dimasukkan*/
						--select idTopping from @promoValid where idPromo = @curPromo2
						insert into @connectedPromoByTopping
						select
							idPromo
						from
							@promoValid
						where
							idTopping in (select idTopping from @promoValid where idPromo = @curPromo2)
	
						--select * from @connectedPromoByTopping
						--update tempDiskonAkhir
						delete from 
							@tempDiskonAkhir
						where
							idPromo in (select idPromo from @connectedPromoByTopping)
